...
public class MitarbeiterHasRollenDAO extends AbstractDAO{
	...
	private final static String GET_MITARBEITER_HAS_ROLLEN_BY_MITARBEITER = "SELECT * FROM mitarbeiter_has_rollen WHERE mitarbeiter_has_rollen.mitarbeiter_fk = {0}";
	...
	public List<MitarbeiterHasRollen> getMitarbeiterHasRollenByMitarbeiter(Mitarbeiter mitarbeiter) throws ConnectException, DAOException, SQLException
	{
		List<MitarbeiterHasRollen> mitarbeiterhasrollen = new  ArrayList<MitarbeiterHasRollen>();
		ResultSet set = getManaged(MessageFormat.format(GET_MITARBEITER_HAS_ROLLEN_BY_MITARBEITER, mitarbeiter.getId()));		
		while(set.next())
		{
			mitarbeiterhasrollen.add(new MitarbeiterHasRollen(set.getLong("mitarbeiter_fk"),
					set.getLong("rollen_fk"))
								);
		}
		return mitarbeiterhasrollen;
