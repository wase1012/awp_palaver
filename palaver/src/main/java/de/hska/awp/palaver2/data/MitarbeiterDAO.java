...
/**
 * Die Klasse stellt Methoden für den Datenbankzugriff für das Objekt Mitarbeiter bereit.
...
public class MitarbeiterDAO extends AbstractDAO{
	...
	/**
	 * Die Methode liefert alle Mitarbeiter aus der Datenbank zurück.
	 * author Christian Barth
	 * @return
	 ...
	 * @throws SQLException
	 */	
	public List<Mitarbeiter> getMitarbeiterByBenutzername(String name)
			throws ConnectException, DAOException, SQLException {

		List<Mitarbeiter> list = new ArrayList<Mitarbeiter>();

		ResultSet set = getManaged(MessageFormat.format(
				GET_MITARBEITER_BY_BENUTZERNAME, name));

		while (set.next()) {
			list.add(new Mitarbeiter(set.getLong(ID), set.getString(NAME), set
					.getString(VORNAME), set.getString(EMAIL), set
					.getString(PASSWORT), set.getString(EINTRITTSDATUM), set
					.getString(AUSTRITTSDATUM), RollenDAO.getInstance()
					.getRollenByMitarbeiterId(set.getLong(ID)), set
	...
	public void createMitarbeiter(Mitarbeiter mitarbeiter)
		...
		String INSERT_QUERY = "INSERT INTO " + TABLE + "(" + NAME + ","
				...
				+ "," + AUSTRITTSDATUM + "," + BENUTZERNAME + ")" + "VALUES"
				+ "('" + mitarbeiter.getName() + "','"
				+ mitarbeiter.getVorname() + "','" + mitarbeiter.getEmail()
		...
		Mitarbeiter mitarbeiterdb = getMitarbeiterById(mitarbeiterlist.get(
				mitarbeiterlist.size() - 1).getId());

		if (mitarbeiter.getRollen() != null) {
			for (int i = 0; i < mitarbeiter.getRollen().size(); i++) {

				MitarbeiterHasRollenDAO.getInstance()
						.createMitarbeiterHasRollen(mitarbeiterdb,
								mitarbeiter.getRollen().get(i));
	...
	/**
	 * Die Methode aktualisiert einen Mitarbeiter mitsamt seiner dazugehörigen Rollen
	 * in der Datenbank.
	 * @param mitarbeiter
	 * @throws ConnectException
	 * @throws DAOException
	...
	public void updateMitarbeiter(Mitarbeiter mitarbeiter)
		...
		String UPDATE_QUERY = "UPDATE " + TABLE + " SET " + NAME + "='"
				...
				+ mitarbeiter.getEintrittsdatum() + "'," + AUSTRITTSDATUM
				+ "='" + mitarbeiter.getAustrittsdatum() + "'," + BENUTZERNAME
				+ "='" + mitarbeiter.getBenutzername() + "' WHERE " + ID + "='"
				+ mitarbeiter.getId() + "'";
		this.putManaged(UPDATE_QUERY);

		if (mitarbeiter.getRollen() != null) {
			for (int i = 0; i < mitarbeiter.getRollen().size(); i++) {
				if (MitarbeiterHasRollenDAO.getInstance()
						.getMitarbeiterHasRollenByMitarbeiterAndRolle(
								mitarbeiter, mitarbeiter.getRollen().get(i)) == null) {
					MitarbeiterHasRollenDAO.getInstance()
							.createMitarbeiterHasRollen(mitarbeiter,
									mitarbeiter.getRollen().get(i));
				}
			}
		}
