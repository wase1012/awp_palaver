...
public class MenueplanDAO extends AbstractDAO {
	...
	private final String GET_ARTIKEL_BEDARF = "select rha.artikel_fk, sum(rha.menge) menge, rha.einheit, mhm.spalte, mhm.portion from rezept_has_artikel rha, menueplan_has_menues mhm, menue_has_rezept mhr where rezept_fk IN "
			...
			" GROUP BY rha.artikel_fk, rha.einheit, mhm.spalte, mhm.portion";
	...
	public List<ArtikelBedarf> getArtikelBedarfByWeek(Week week)
		...
		ResultSet set = getManaged(MessageFormat.format(GET_ARTIKEL_BEDARF,
		...
		while (set.next()) {
			...
			Mengeneinheit einheit = Mengeneinheitverwaltung.getInstance()
					.getMengeneinheitById(einheitId);
	...
	public Menueplan getMenueplanItems(Week week) throws ConnectException,
		...
		while (setMpl.next()) {
			...
			mpl.setFreigegeben(setMpl.getBoolean("freigegeben"));
			System.out.println(mpl.getFreigegeben());
		}
		...
		if (mpl != null) {
			...
			while (setMenues.next()) {
				...
				Geschmack geschmack = getGeschmackById(setMenues
						.getLong("geschmack_fk"));
				menue.setGeschmack(geschmack);

				Menueart menueart = getMenueartById(setMenues
						.getLong("menueart_fk"));
	...
	public Menueplan getMenueplanByWeekWithItems(Week week)
		...
		openConnection();
		ResultSet setMpl = getMany(MessageFormat.format(GET_MENUEPLAN_BY_WEEK,
				week.getWeek(), week.getYear()));

		while (setMpl.next()) {
			menueplan = new Menueplan(setMpl.getLong(ID), week);
			menueplan.setFreigegeben(setMpl.getBoolean("freigegeben"));
			System.out.println(menueplan.getFreigegeben());
		}
		...
		if (menueplan != null) {
			...
			List<MenueComponent> menues = new ArrayList<MenueComponent>();
			// TODO: Menüs laden
			ResultSet setMenues = getMany(MessageFormat.format(
	...
	private Menueart getMenueartById(Long id) throws ConnectException,
			DAOException, SQLException {
	...
	private Geschmack getGeschmackById(Long id) throws ConnectException,
			DAOException, SQLException {
		Geschmack geschmack = null;
		ResultSet set = getMany(MessageFormat.format(GET_GESCHMACK_BY_ID, id));
		while (set.next()) {
	...
	private List<Fussnote> getFussnoteByMenue(Long id) throws ConnectException,
			DAOException, SQLException {
		List<Fussnote> list = new ArrayList<Fussnote>();
		ResultSet set = getMany(MessageFormat.format(GET_FUSSNOTE_BY_MENUE, id));
		while (set.next()) {
	...
	public Fussnote getFussnoteByIdMany(Long id) throws ConnectException,
			DAOException, SQLException {
		Fussnote fussnote = null;
		ResultSet set = getMany(MessageFormat.format(GET_FUSSNOTE_BY_ID, id));
		while (set.next()) {
	...
	private List<Rezept> getRezepteByMenue(Menue menue)
			throws ConnectException, DAOException, SQLException {
		List<Rezept> rezepte = new ArrayList<Rezept>();
		ResultSet set = getMany("select rez.* from menue_has_rezept mhr, rezept rez where mhr.rezept_id=rez.id and mhr.menue_id = "
				+ menue.getId());
		while (set.next()) {
	...
	private List<RezeptHasArtikel> ladeArtikelFuerRezept(Rezept rez)
			throws ConnectException, DAOException, SQLException {
	...
	public void createMenueForMenueplan(Menueplan mpl, Menue menue,
			String name, int col, int row, Integer portion)
	...
	public void updateMenueplan(Menueplan menueplan) throws ConnectException,
			DAOException {
		putManaged(MessageFormat.format(UPDATE_MENUEPLAN_FREIGEBEN,
				menueplan.getFreigegeben(), menueplan.getId()));
	}
	...
	public List<KochInMenueplan> getKoecheByMenueplan(Menueplan menueplan)
		...
		String sql = "SELECT * FROM menueplan_has_koeche WHERE "
				+ menueplan.getId();
		...
		while (set.next()) {
			...
			k.setKoch(Mitarbeiterverwaltung.getInstance().getMitarbeiterById(
					kochId));
